...
package frc.lib2.pathfinder.pathCreator;
...
public class PathGenerator2 {
	...
	private static final long serialVersionUID = 1L;
	   ...
	   public static ArrayList<Point2> newPoints = new ArrayList<Point2>();

	   public static ArrayList<Vector2> newVectors = new ArrayList<Vector2>();

	   public static ArrayList<Integer> newNumOPoints = new ArrayList<Integer>();

	   public static ArrayList<Point2> finalPoints = new ArrayList<Point2>();
...
public static void createDataSet(){
	...
	// addPoint(7, 3.32);
	   ...
	   for(int i = 0; i < newPoints.size() - 1; i++) {
		  newVectors.add(new Vector2(newPoints.get(i+1).getX() - newPoints.get(i).getX(), newPoints.get(i+1).getY() - newPoints.get(i).getY()));
	   }
...
//	   }
	   for (int i = 0; i < newNumOPoints.size(); i++) {
		   for(int j = 0; j < newNumOPoints.get(i); j++) {
			   finalPoints.add(new Point2(newPoints.get(i).getX() + newVectors.get(i).getX() * j, 
